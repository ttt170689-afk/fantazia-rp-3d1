  ...
  window.FZ_TEASER_MALL = {
    ...
    build: function (scene, ox, oy, oz, opts) {
      ...
      function buildMALL() {
          ...
          // ── КОЛОННЫ (как в игре) ──
          // v45: колонна [15, -30] УБРАНА. После сдвига группы (z += 22) она
          // попадала ровно в (15, -8) — вплотную к дверям лифта (16, -8) —
          // и физически перекрывала вход. Плюс она дублировала колонну,
          // которую строит сам buildMallInterior.
          [[-15, -30], [0, -30], [-15, -13], [0, -13], [15, -13]].forEach(([cx, cz]) => {
          ...
          (function buildServiceComplex() {
            ...
            // ── НИША ЗА ВОРОТАМИ ──
            // v48: БЕТОННЫЕ СТУПЕНИ УБРАНЫ по просьбе игрока («убери
            // лестницы»). Это была декоративная лестница, уходящая вниз
            // за служебными воротами B1. Сама ниша, подсветка и табличка
            // «B1 ↓» остаются, чтобы за воротами не зияла пустая стена;
            // вместо ступеней — ровная площадка пола.
            const nicheM = new THREE.MeshLambertMaterial({ color: 0x1b1e24 });
            B.add(box(1.2, 0.15, 3.2, NM.conc2, 22.3, -0.05, qz));   // площадка
            B.add(box(4.6, 0.15, 3.2, NM.conc2, 24.6, -0.05, qz));   // ровный пол ниши
